...
function generatePostHtml(data, allPosts = []) {
  ...
  const promoDropHtml = (data.promo_drop && data.promo_drop.code) ? `
    <div class="blog-promo-drop-card" style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.12) 0%, rgba(249, 115, 22, 0.08) 100%); border: 2px solid #FCD34D; border-radius: 20px; padding: 1.5rem; margin: 2rem 0; box-shadow: 0 4px 14px rgba(245, 158, 11, 0.1);">
      <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 1rem;">
        <div>
          ...
          <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem; margin-top: 0.5rem;">
            <span style="font-size: 0.875rem; color: #475569;">Use secret code:</span>
            <button
              type="button"
              id="copyPromoBtn"
              onclick="navigator.clipboard.writeText('${data.promo_drop.code}').then(() => { this.innerText = '✓ Copied!'; setTimeout(() => { this.innerText = '${data.promo_drop.code} 📋'; }, 2000); })"
              style="font-family: monospace; font-weight: 900; font-size: 0.9rem; background: #FFFFFF; border: 2px solid #FCD34D; color: #78350F; padding: 0.25rem 0.6rem; border-radius: 8px; cursor: pointer;"
              title="Click to copy promo code"
            >${data.promo_drop.code} 📋</button>
          </div>
        </div>
        <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem;">
          <button
            type="button"
            onclick="if (navigator.share) { navigator.share({ title: 'Kibo Climb Secret Reader Drop', text: 'Use secret code ${data.promo_drop.code} on Kibo Climb for +${data.promo_drop.sparks || 100} bonus Sparks! 🏔️🐾', url: window.location.origin + '/?action=workshop&promo=${encodeURIComponent(data.promo_drop.code)}' }); } else { navigator.clipboard.writeText(window.location.origin + '/?action=workshop&promo=${encodeURIComponent(data.promo_drop.code)}').then(() => alert('Link copied to clipboard!')); }"
            style="padding: 0.75rem 1rem; font-size: 0.875rem; font-weight: 700; background: #FFFFFF; border: 2px solid #FCD34D; color: #78350F; border-radius: 14px; cursor: pointer;"
          >Share Drop ↗</button>
          <a href="/?action=workshop&promo=${encodeURIComponent(data.promo_drop.code)}" class="cta-button" style="padding: 0.75rem 1.25rem; font-size: 0.875rem; margin-top: 0; text-decoration: none;">Redeem in Game →</a>
        </div>
      </div>
    </div>` : '';
