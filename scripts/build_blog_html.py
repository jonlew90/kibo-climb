...
def markdown_to_html(md_text: str) -> str:
    ...
    for block in blocks:
        ...
        if first_line.startswith("### "):
            ...
            if len(lines) > 1:
                rem_block = "\n".join(lines[1:])
                html_parts.append(markdown_to_html(rem_block))
...
def generate_post_html(data: dict) -> str:
    """Renders a single blog post into full static HTML using public/css/blog.css."""
    slug = data["slug"]
    title = data["title"]
...
<body>
  ...
  <main class="article-container">
    ...
    <img src="{featured_image_url}" alt="{title}" class="hero-img" />
...
def update_sitemap(posts: list[dict]):
    """
    Updates public/sitemap.xml with all blog posts.
    CRITICAL SEO REQUIREMENT: <lastmod> is derived strictly from fixed published_at date,
    never from the runtime datetime.now().
    """
    ...
    # Clean whitespace from existing nodes before re-indenting
    def clean_elem(elem):
        if elem.text:
