...
export default function KiboBreakOverlay({
  ...
  return (
    <div className="fixed inset-0 z-[999] h-[100dvh] max-h-[100dvh] bg-gradient-to-b from-amber-50 via-sky-50 to-teal-50 text-slate-800 flex flex-col justify-start items-center p-3 pt-2 pb-3 sm:p-4 overflow-y-auto select-none animate-pop">
      <ConfettiCanvas />

      <div className="w-full max-w-sm sm:max-w-md mx-auto space-y-2 relative z-10 text-center py-1">
        {/* Banner Header */}
        ...
        <div className="relative py-0.5 flex justify-center items-center">
          <div className="absolute w-24 h-24 rounded-full bg-amber-400/25 blur-xl animate-pulse pointer-events-none" />
          <Mascot
            mood="happy"
            state="break"
            equipped={equippedItems}
            className="w-18 h-18 sm:w-22 sm:h-22 filter drop-shadow-md animate-bounce"
          />
        </div>

        {/* 4-Tile Detailed Climb Stats Matrix */}
        <div className="grid grid-cols-2 gap-1.5 bg-white/95 border-2 border-amber-200/90 rounded-2xl p-2 backdrop-blur-md shadow-md text-left">
          {/* Accuracy Tile */}
          <div className="bg-emerald-50/80 border border-emerald-200/80 rounded-xl p-1.5 space-y-0.5">
            ...
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-black text-emerald-700">{correctCount} / {totalCount}</span>
              <span className="text-[10px] font-extrabold text-emerald-600">{accuracyPct}%</span>
            </div>
          </div>

          {/* Sparks Earned Tile */}
          <div className="bg-amber-50/80 border border-amber-200/80 rounded-xl p-1.5 space-y-0.5">
          ...
          {/* Streak Boost Tile */}
          <div className="bg-orange-50/80 border border-orange-200/80 rounded-xl p-1.5 space-y-0.5">
        ...
        <div className="bg-gradient-to-r from-amber-100 via-yellow-100 to-purple-100 border border-amber-300 rounded-2xl p-1.5 px-3 flex items-center justify-between shadow-xs">
        ...
        <div className="space-y-1.5 pt-0.5">
          <button
            ...
            className="btn-3d-orange w-full py-2.5 sm:py-3 text-base sm:text-lg font-black rounded-xl flex items-center justify-center gap-2 shadow-bouncy-orange active:scale-95 transition-transform"
          >
