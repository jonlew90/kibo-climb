...
export default function MathSessionView({
  ...
  const handlePassQuestion = () => {
    ...
    if (!isAlreadyReview) {
      ...
      storageService.addToPracticeQueue(skippedProblem, 'math');
    }
  ...
  return (
    <div className="w-full h-full flex-1 min-h-0 relative overflow-visible animate-pop flex flex-col">
      {/* TRAINING CAMP IDENTITY BANNER — visible whenever a practice session is active */}
      {isPracticeMode && (
                  ...
                  {incorrectReviewData ? (
                    ...
                    <>
                      ...
                      >
                        ...
                        <span>{showFrustrationCard ? 'Active' : isPracticeMode ? 'Hint (Free)' : (consumables?.hintScrollCount ?? 0) > 0 ? `Hint (${consumables.hintScrollCount})` : 'Hint +'}</span>
                      </button>

                      {/* CLIMBER SPYGLASS BUTTON (Only when owned or active, hidden in practice mode) */}
                      {!isPracticeMode && ((consumables?.letterSpyglassCount ?? 0) > 0 || spyglassRevealedAnswer) && (
                        <button
                          type="button"
                      ...
                      {/* CLIMBER PRUNER BUTTON (Only when owned or active, hidden in practice mode) */}
