      ...
      <main className="flex-1 min-h-0 overflow-y-auto custom-scrollbar touch-pan-y overscroll-contain w-full max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        {activeTab === 'overview' && (
          <div className="flex-1 space-y-4 my-1">
            ...
            {/* KIBO CLUB ACTIVE MEMBERSHIP STATUS BADGE (Only for active subscribers) */}
            {(() => {
              const hasFamily = storageService.hasFamilyPlan();
              const hasSingle = storageService.hasSinglePlan(viewingProfileId);
              const trialStatus = storageService.getTrialStatus(viewingProfileId);
              const isSubscribed = hasFamily || hasSingle;

              if (!isSubscribed && !trialStatus.isTrial) return null;

              if (trialStatus.isTrial && !trialStatus.isExpired) {
                return (
                  <div className="rounded-2xl p-3.5 border-2 shadow-sm text-left relative overflow-hidden transition-all bg-gradient-to-r from-amber-500/15 via-orange-500/10 to-amber-500/15 border-amber-300">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-700 leading-snug font-medium">
                          Enjoying 1.25x Sparks, 15% VIP discounts, and printable worksheets? Upgrade to an official Solo or Family plan to keep these perks active without interruption.
                        </p>
                      </div>
                      <div className="shrink-0">
                        <button
                          ...
                          className="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white text-xs font-black rounded-xl shadow-xs transform active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                        >
                          <Sparkles className="w-3.5 h-3.5 fill-white" />
                          <span>Keep Membership</span>
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              return (
                <div className={`rounded-2xl p-3.5 border-2 shadow-sm text-left relative overflow-hidden transition-all ${
                  hasFamily
                    ...
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full flex items-center gap-1 ${
                          ...
                          <Sparkles className="w-3 h-3 fill-current" />
                          {hasFamily ? 'Kibo Club Family Plan' : 'Kibo Club Member'}
                        </span>
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs font-black uppercase text-indigo-950 tracking-wider">
                              NWEA MAP® Growth Alignment*
                            </span>
                          </div>
                          <span
                            title="Informational estimate based on published national norms. NWEA® and MAP® are registered trademarks of NWEA/HMH, which does not sponsor or endorse Kibo Climb. See Terms."
                            className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200 cursor-help"
                          >
                            National Norms Reference*
                          </span>
                        </div>

                        {/* Free Benchmark Callout */}
                        <div className="bg-gradient-to-r from-indigo-50/80 to-purple-50/80 border border-indigo-200/80 rounded-xl p-3 flex items-center justify-between gap-3">
                          <div className="space-y-0.5">
                            <div className="text-xs font-bold text-slate-600">
                              Estimated MAP Math Range:
                            </div>
                            <div className="text-base sm:text-lg font-black text-indigo-950 flex items-center gap-2">
                              <span>RIT {ritInfo.bandString}</span>
                              <span className="text-xs font-bold text-indigo-700 bg-indigo-100/70 px-2 py-0.5 rounded-md">
                                {ritInfo.gradeBand}
                              </span>
                            </div>
                          ...
                          <div className="text-right shrink-0">
                            <span className="text-[10px] font-black uppercase text-emerald-800 bg-emerald-100 border border-emerald-300 px-2 py-0.5 rounded-full block text-center">
                              Active
                            </span>
                            <span className="text-[10px] text-slate-400 block mt-0.5">Based on play</span>
                          </div>
                        </div>

                        {/* 4-Domain Breakdown: Fully unlocked for Kibo Club; Teased with blur & Lock for Free */}
                        <div className="space-y-1.5 pt-1">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-black uppercase text-slate-700 tracking-wider flex items-center gap-1">
                              <span>📊</span> 4 Core Instructional Domains
                            </span>
                            {!isMember && (
                          ...
                          <div className="relative rounded-xl overflow-hidden">
                            <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2 ${!isMember ? 'filter blur-[1.5px] pointer-events-none select-none opacity-60' : ''}`}>
                              {domainBreakdown.map((dom) => (
                                <div key={dom.id} className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1">
                                  <div className="flex items-center justify-between text-xs gap-1">
                                    <div className="flex items-center gap-1.5 min-w-0">
                                      <span className="text-sm shrink-0">{dom.icon}</span>
                                      <span className="font-extrabold text-slate-800 truncate text-xs">{dom.shortName}</span>
                                    </div>
                                    <span className={`text-[10px] font-black uppercase px-1.5 py-0.2 rounded border shrink-0 ${
                                      dom.status.includes('Proficient')
                                        ? 'bg-emerald-100 text-emerald-800 border-emerald-300'
                                        : dom.status.includes('Developing')
                                        ? 'bg-amber-100 text-amber-800 border-amber-300'
                                        : 'bg-slate-200 text-slate-700 border-slate-300'
                                    }`}>
                                      {dom.accuracyPct !== null ? `${dom.accuracyPct}%` : dom.status}
                                    </span>
                                  </div>
                            ...
                            {!isMember && (
                              <div className="absolute inset-0 bg-indigo-950/40 backdrop-blur-[1px] rounded-xl flex flex-col items-center justify-center p-3 text-center text-white space-y-1.5 shadow-sm">
                                <div className="flex items-center gap-1.5 text-amber-300 text-xs font-black">
                                  <Lock className="w-3.5 h-3.5" />
                                  <span>Unlock Sub-Domain Diagnostic Breakdown</span>
                                </div>
                                <p className="text-[11px] text-slate-200 max-w-xs leading-snug">
                                  Pinpoint performance across Operations, Fractions & Decimals to target school test prep.
                                </p>
                                <button
                                  type="button"
                                  onClick={() => {
                                  ...
                                  className="px-3.5 py-1 bg-amber-400 hover:bg-amber-500 text-amber-950 text-xs font-black rounded-lg transition-transform active:scale-95 shadow-sm cursor-pointer flex items-center gap-1"
                                >
                                  <Sparkles className="w-3 h-3 fill-current" />
            ...
            {(() => {
              ...
              const isMember = storageService.hasClubMembership(viewingProfileId);
              const insightCards = isMember ? generateParentInsightCards(skipLogs, sprintHistory, profileName, selectedSubject) : [];
              ...
              return (
                <section className="bg-white rounded-2xl p-4 border-2 border-purple-200 text-left space-y-4 shadow-xs">
                  ...
                  {/* ACTIONABLE INSIGHT CARDS (KIBO CLUB) */}
                  {isMember ? (
                    <div className="space-y-2">
                      {insightCards.length > 0 ? (
                        ...
                        <div className="bg-emerald-50/60 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-900 font-medium flex items-center gap-2">
                          <span className="text-lg">🌟</span>
                          <span>No bottlenecks detected! {profileName} is demonstrating strong pacing and consistent accuracy.</span>
                        </div>
                      )}
                    </div>
                  ) : (
                    /* STANDARDIZED LOCKED TEASER FOR NON-CLUB USERS */
                    <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-xl p-4 sm:p-5 border border-indigo-500/30 shadow-md">
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed mb-3">
                        Get automated topic bottleneck detection, fatigue alerts, and actionable parent coaching recommendations tailored to {profileName}'s daily climbs.
                      </p>
                      <div className="flex justify-center pt-1">
                        <button
                          ...
                          className="px-5 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-black text-xs rounded-xl shadow-md transition-transform active:scale-95 flex items-center gap-1.5 cursor-pointer"
                        >
                          <span>👑</span> Upgrade to Kibo Club
                        </button>
                      </div>
                    </div>
                  )}
        ...
        {activeTab === 'verification' && (
          <div className="flex-1 space-y-4 my-1">
            ...
            {(() => {
              ...
              const hasSingle = currentPlan?.tier === 'single' && !currentPlan?.isTrial;
              const isTrialActive = trialStatus.isTrial && !trialStatus.isExpired;
              ...
              return (
                ...
                >
                  {/* Trial Active Expiration Alert Banner */}
                  {isTrialActive && (
                    <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 text-white p-3.5 rounded-xl shadow-xs space-y-1">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <p className="text-[11px] text-amber-100 font-medium leading-relaxed">
                        Your free trial unlocks all Kibo Club perks. Select an annual or monthly plan below to transition smoothly without losing 1.25x Sparks, VIP discounts, or printable worksheet access.
                      </p>
                    </div>
                  )}
                ...
                <div className="space-y-3">
                      ...
                      {digest.isKiboClub && sub.clubInsights?.insightCards && sub.clubInsights.insightCards.length > 0 && (
                        <div className="bg-amber-50/70 border border-amber-200/80 rounded-xl p-3 space-y-2 mt-2">
                          <div className="flex items-center gap-1.5 text-xs font-black text-amber-900 uppercase tracking-wider">
                            <span>👑</span> Kibo Club Diagnostic Insights
                          </div>
                          <div className="space-y-1.5">
                  ...
                  {!digest.isKiboClub && (
                    <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-xl p-4 sm:p-5 border border-indigo-500/30 shadow-md">
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed mb-3">
                        Get automated topic bottleneck detection, fatigue alerts, and actionable parent coaching recommendations tailored to {digest.childName}'s daily climbs.
                      </p>
                      <div className="flex justify-center pt-1">
                        <button
                          type="button"
