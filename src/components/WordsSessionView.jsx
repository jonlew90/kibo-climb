            ...
            return (
              ...
              >
                ...
                <div className="w-full flex items-center justify-between gap-1 sm:gap-2 px-1 py-0.5 text-xs">
                  ...
                  <span className="font-black uppercase text-purple-700 bg-purple-50 px-2 sm:px-2.5 py-1 rounded-full border border-purple-200 shrink-0 shadow-2xs text-[10px] sm:text-xs">
                    🎯 Q #{currentQuestionNum}/12
                  </span>

                  {/* Center: Priority Status Badge (Probe > Gatekeeper > Streak) */}
                  {currentProblem.isProbe ? (
                    <span className="font-black uppercase text-white bg-gradient-to-r from-amber-500 to-indigo-600 px-2.5 py-1 rounded-full border border-indigo-300 shrink-0 shadow-xs animate-pulse flex items-center gap-1 text-[10px] sm:text-xs">
                      🚀 PROBE (+120)
                    </span>
                  ) : isNearTierThreshold(competenceRank) ? (
                    <span className="font-black uppercase text-amber-950 bg-gradient-to-r from-amber-300 to-yellow-400 px-2.5 py-1 rounded-full border border-amber-500 shrink-0 shadow-xs animate-pulse flex items-center gap-1 text-[10px] sm:text-xs" title="1 question away from entering the next Tier!">
                      ⚡ GATEKEEPER
                    </span>
                  ) : (
                    <span className={`font-black uppercase px-2.5 py-1 rounded-full border shrink-0 transition-all duration-300 shadow-2xs text-[10px] sm:text-xs ${streakCfg.pillClass}`}>
                      {streakCfg.label}
                    </span>
                  )}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <span
                      className={`font-black uppercase px-2 sm:px-2.5 py-1 rounded-full border shadow-2xs flex items-center gap-1 text-[10px] sm:text-xs ${
                        storageService?.hasClubMembership?.(profileId)
                    ...
                    >
                      ...
                      {isDoubleSparksActive && (
                        <span className="text-[9px] sm:text-[10px] bg-amber-400 text-amber-950 px-1 rounded font-black leading-none">2x</span>
                      )}
                    </span>

                    {((consumables?.shieldCount || 0) > 0 || (consumables?.streakSaverCount || 0) > 0) && (
                      <span
                        className="font-black uppercase text-sky-950 bg-sky-100 px-1.5 sm:px-2 py-1 rounded-full border border-sky-300 shadow-2xs flex items-center gap-0.5 cursor-help text-[10px] sm:text-xs"
                        title="Kibo Shield Active: Streak protected!"
                      >
                        🛡️ <span>{consumables.shieldCount || consumables.streakSaverCount}</span>
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="w-full flex items-center justify-center gap-1 sm:gap-1.5 py-0.5">
                  {incorrectReviewData ? (
                    <span className="text-[10px] sm:text-xs font-black uppercase text-rose-800 bg-rose-100 px-3 py-1 rounded-full border border-rose-300 shadow-2xs font-extrabold flex items-center gap-1 animate-pulse">
                      ❌ Reviewing Solution
                    </span>
                  ) : (
                    <>
                      ...
                      <button
                        ...
                        className={`text-[10px] sm:text-xs font-black uppercase px-2 sm:px-2.5 py-1 rounded-full border shrink-0 transition-all active:scale-95 flex items-center gap-1 ${
                          consecutiveSkips >= 2
                      ...
                      <button
                        ...
                        className={`text-[10px] sm:text-xs font-black uppercase px-2 sm:px-2.5 py-1 rounded-full border shrink-0 transition-all active:scale-95 flex items-center gap-1 cursor-pointer ${
                          showFrustrationCard
